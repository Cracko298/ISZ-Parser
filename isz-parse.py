...
# Start of Arguments
if len(sys.argv) > 0:
    length_check = len(sys.argv) - 1
    base_name = basename(__file__)
    num_ranges = range(0,256)
    sys_list = sys.argv

    if length_check >= 3:
        print(f"Invalid Number of Args Given.")
        print(f"Arguments Recieved: '{length_check}'.")
        sys.exit(1)

    if base_name != f'isz-parse.py' or base_name != f'isz-parse.exe':
        for num in num_ranges:
            if base_name == f'isz-parse({num}).py' or base_name == f'isz-parse({num}).exe':
                print(f"Verified: '{base_name}' (No '--f'/'--force' Flag Required).\n")
                verify = 1
                break
        ...
        if check_number >= 255:
            ...
            for i, arg in enumerate(sys.argv):
                if arg in arg_to_find:
                    position = i
                    break
        
            if '--f' not in sys_list and '--force' not in sys_list:
                print(f"Modified ISZ-Parse Detected.")
                print(f"File Name: '{base_name}'.")
                sys.exit(1)

    if base_name in sys_list:
        sys_list.remove(f'{base_name}')
